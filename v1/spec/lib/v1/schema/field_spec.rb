    ...
    describe Field do
      ...
      describe "sorting" do
        it "implements correct sort and sortable? for sort:field" do
          field = V1::Schema::Field.new(resource, 'id', item_mapping['id'])
        ...
        it "implements correct sort and sortable? for un-sortable field" do
          field = V1::Schema::Field.new(resource, 'description', item_mapping['description'])
      ...
      describe "#multi_fields" do
        it "returns empty array when there are no mult_fields to build" do
          field = V1::Schema::Field.new(resource, 'title', item_mapping['title'])
          expect(field.multi_fields).to eq []
        end
        it "returns array of multi_fields when there are one or more multi_fields to build" do
          field = V1::Schema::Field.new(resource, 'isPartOf.name', item_mapping['isPartOf']['properties']['name'])
      ...
      describe "#subfields" do
        ...
        it "handles subfields for multi_field type" do
          field = V1::Schema::Field.new(resource, 'isPartOf', item_mapping['isPartOf'])
      ...
      describe "#facetable?" do
        context "facetable fields" do
          it "detects a top level simple field" do
            field = V1::Schema::Field.new(resource, 'created', item_mapping['created'])
            expect(field.facetable?).to be_true
          end
          it "detects a subfield" do
            field = V1::Schema::Field.new(resource, 'spatial.iso3166-2', item_mapping['spatial']['properties']['iso3166-2'])
            expect(field.facetable?).to be_true
          end
        ...
        context "non-facetable fields" do
          ...
          it "detects a subfield" do
            field = V1::Schema::Field.new(resource, 'subject.@id', item_mapping['subject']['properties']['@id'])
            expect(field.facetable?).to be_false
          end
          it "detects a multi_field types with a 'raw' subfield" do
            field = V1::Schema::Field.new(resource, 'field1.name', item_mapping['field1']['properties']['name'])
            expect(field.facetable?).to be_false
          end
        end
      ...
      describe "#enabled?" do
        it "returns false if a field is explicitly not enabled" do
          field = V1::Schema::Field.new(resource, 'disabledField', item_mapping['disabledField'])
